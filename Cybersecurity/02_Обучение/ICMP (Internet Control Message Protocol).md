...
> ICMP — это служебный протокол сетевого уровня (L3 по [[Модель OSI  (Open Systems Interconnection)|модели OSI]]), который не передаёт пользовательские данные, а доставляет **управляющие сообщения**: ошибки, диагностику и информацию о состоянии сети. Если IP — это почтовая служба, доставляющая письма, то ICMP — это служебные уведомления: «адресат не найден», «письмо слишком большое», «время доставки истекло».
...
0	Echo Reply	Ответ на запрос ping. Говорит: «Я здесь!».	ping
3	Destination Unreachable	Целевой хост или сеть недоступны. Код уточняет причину.	—
